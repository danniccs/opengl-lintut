...
SimpleMesh::SimpleMesh(const float *vertices, unsigned int numVertices,
                       vector<string> texturePaths, bool bSRGB,
                       bool hasTangents, bool hasNormals,
  ...
  size_t normSize = 0;
  size_t tanSize = 0;
  size_t coordSize = 2 * sizeof(float);

  if (hasNormals)
    normSize = 3 * sizeof(float);
  if (hasTangents)
    tanSize = 3 * sizeof(float);

  GLsizei totalSize =
      static_cast<GLsizei>(posSize + normSize + coordSize + tanSize);

  glBindBuffer(GL_ARRAY_BUFFER, VBOs[POS_NORM_TEX_TAN_VB]);
  glBufferData(GL_ARRAY_BUFFER, numVertices * static_cast<size_t>(totalSize),
  ...
  if (hasNormals) {
    ...
    glVertexAttribPointer(NORM_LOC, 3, GL_FLOAT, GL_FALSE, totalSize,
                          (void *)(posSize));
  }

  // Texture coordinate attribute
  glEnableVertexAttribArray(TEX_LOC);
  glVertexAttribPointer(TEX_LOC, 2, GL_FLOAT, GL_FALSE, totalSize,
                        (void *)(posSize + normSize));

  if (hasTangents) {
    // Normal attribute
    glEnableVertexAttribArray(TAN_LOC);
    glVertexAttribPointer(TAN_LOC, 3, GL_FLOAT, GL_FALSE, totalSize,
                          (void *)(posSize + normSize + coordSize));
  }
  ...
  for (unsigned int i = 0; i < 4; i++) {
    ...
    glVertexAttribPointer(MOD_LOC + i, 4, GL_FLOAT, GL_FALSE, sizeof(glm::mat4),
  ...
  if (hasNormals) {
    ...
    for (unsigned int i = 0; i < 3; i++) {
      ...
      glVertexAttribPointer(NORM_M_LOC + i, 3, GL_FLOAT, GL_FALSE,
                            sizeof(glm::mat3), (void *)(sizeof(glm::vec3) * i));
...
void SimpleMesh::Draw(Shader shader, unsigned int numInstances,
                      glm::mat4 *models, glm::mat3 *normMats) {
  // Populate model matrices
  if (models != NULL) {
    glBindBuffer(GL_ARRAY_BUFFER, VBOs[MOD_VB]);
    glBufferData(GL_ARRAY_BUFFER, numInstances * sizeof(glm::mat4), models,
                 GL_DYNAMIC_DRAW);
  }

  if (normMats != NULL) {
    glBindBuffer(GL_ARRAY_BUFFER, VBOs[NORM_M_VB]);
    glBufferData(GL_ARRAY_BUFFER, numInstances * sizeof(glm::mat3), normMats,
                 GL_DYNAMIC_DRAW);
  }
...
vector<Texture> SimpleMesh::loadTextures(vector<string> texturePaths,
  ...
  for (unsigned int i = 0; i < texturePaths.size(); i++) {
    ...
    if (texParams.empty())
      ...
      texture.id =
          loadTexture(path, bSRGB, true, sWrap, tWrap, minFilter, magFilter);
    }
    texture.location = 0;
    texture.path = path;
